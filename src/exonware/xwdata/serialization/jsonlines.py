...
class JSONLinesSerializer(AXWDataSerializer):
    ...
    async def serialize(self, data: Any, **opts) -> str:
        ...
        try:
            ...
            if isinstance(data, list):
                lines = []
                for item in data:
                    line = JsonSerializer().encode(item, options={'ensure_ascii': False})
                    lines.append(line)
                return '\n'.join(lines)
            # If single object, serialize as single line
            return JsonSerializer().encode(data, options={'ensure_ascii': False})
    ...
    async def deserialize(self, content: str | bytes, **opts) -> list[Any]:
        """
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                try:
                    obj = JsonSerializer().decode(line)
                    results.append(obj)
                except SerializationError as e:
                    raise XWDataParseError(
                        f"Invalid JSON on line {i+1}: {str(e)}",
                        format='jsonlines',
                        line=i+1
                    ) from e
            return results
