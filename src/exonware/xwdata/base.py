...
class AData(IData):
    ...
    def __str__(self) -> str:
        ...
        try:
            ...
            return JsonSerializer().encode(native_data, options={'indent': 2, 'ensure_ascii': False, 'default': str})
...
class ADataEngine(IDataEngine):
    ...
    async def _validate_path(self, path: str | Path, for_writing: bool = False) -> Path:
        ...
        # Security validation - reuse xwdata PathValidator (single place for allow_absolute etc.)
        if self._config and hasattr(self._config, 'security'):
            if self._config.security.enable_path_validation:
                from exonware.xwdata.core.validators import get_path_validator
                validator = get_path_validator()
                # If path was originally relative, validate using relative form
                op = "save" if for_writing else "load"
                if was_relative:
                    try:
                        relative_for_validation = path_obj.relative_to(Path.cwd())
                        validator.validate_path(str(relative_for_validation), operation=op)
                    except ValueError:
                        validator.validate_path(str(path), operation=op)
                else:
                    validator.validate_path(str(path_obj), operation=op)
        return path_obj
