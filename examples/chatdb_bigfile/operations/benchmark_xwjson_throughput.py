...
async def benchmark_path_read_throughput(
    ...
    # Warmup: read once to populate cache
    await ops.read_path(file_path, "/records/0/@type")
    
    # Benchmark path reads
    start_time = time.perf_counter()
    
    for i in range(num_operations):
        path = f"/records/{i % 100}/@type"
...
async def benchmark_paging_throughput(
    ...
    # Warmup: read first page to populate cache
    await ops.read_page(file_path, page_number=1, page_size=page_size, path="/records")
    ...
    for i in range(num_operations):
        ...
        page = await ops.read_page(file_path, page_number=page_num, page_size=page_size, path="/records")
