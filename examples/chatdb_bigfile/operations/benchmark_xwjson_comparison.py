...
async def benchmark_xwjson_load(xwjson_path: Path) -> dict[str, Any]:
    ...
    # Data can be a list or dict with "records" key
...
async def benchmark_xwjson_paging(xwjson_path: Path, num_pages: int = 10) -> dict[str, Any]:
    ...
    # First page read (loads file and builds cache) - data is a list, so path is None
    first_page_start = time.perf_counter()
    first_page = await ops.read_page(xwjson_path, page_number=1, page_size=100, path=None)
    ...
    for page_num in range(2, num_pages + 1):
        ...
        page = await ops.read_page(xwjson_path, page_number=page_num, page_size=100, path=None)
